...
const Form: React.FC<FormProps> = (props) => {
  ...
  let submitButton = <button onClick={props.onSubmit} disabled={!isInputValid2} className="mt-8 bg-gradient-to-r from-sky-400 to-sky-800 text-white text-lg p-2 w-full rounded-md disabled:opacity-50 disabled:bg-gray-500">Submit</button>
  if (props.isLoading) {
    submitButton =  <button type="button" disabled={props.isLoading}  className="mt-8 text-white text-lg p-2 w-full rounded-md disabled:opacity-50 disabled:bg-slate-900">
                      <svg aria-hidden="true" className="inline w-6 h-6 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
