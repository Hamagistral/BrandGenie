...
const Form: React.FC<FormProps> = (props) => {
  ...
  if (props.isLoading) {
    submitButton =  <button type="button" disabled={props.isLoading}  className="mt-8 text-white font-semibold text-lg p-2 w-full rounded-md bg-gradient-to-r from-sky-500 to-sky-800">
                      <svg aria-hidden="true" className="inline w-6 h-6 mr-2 text-gray-400 animate-spin fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <>
      <div>
        <p className="mt-6 font-light text-gray-200">Get a complete brand package for your e-commerce site with just a product description. Includes name, slogan, ad copy, and related keywords.</p>
        <input 
          type="text"
          placeholder="Watches"
          value={props.input}
